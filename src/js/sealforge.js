...
function SF_initializeEventListeners() {
  ...
  document.querySelector('.sf-reset').addEventListener('click', SF_resetFormHandler);
}
...
function SF_handleConcatenatablesChange(context) {
  ...
  concatenatableInputs.forEach(input => {
    if (input.value) {
      ...
      growingMarkupString += SF_generateHtmlSegment(input, countOfConcatenatablesWithContent);
    }
  });
...
function SF_getSocialLinks() {
  return {
    ...
    'sf-linkedin': 'https://www.linkedin.com/school/lincoln-university-of-missouri/',
